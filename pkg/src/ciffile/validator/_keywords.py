...
def item_default(df: tools.CIFItemCategoryDataFrame) -> pl.DataFrame:
    ...
    if duplicated.any():
        ...
        df = df.unique()
    return (
        rename_frame_code(df)
        .select(["category", "keyword", "value"])
        .sort(by=["category", "keyword"])
    )
...
def process_df_item(df: pl.DataFrame):
    ...
    with pl.StringCache():
        ...
        if (
            not df_1["mandatory_code"]
            .unique()
            .is_in(["YES", "NO", "IMPLICIT", "IMPLICIT-ORDINAL"])
            .all()
        ):
    ...
    if match_names(df_1, col_name="name", reduce=True):
        ...
        if not match_cat_key(df_2, col_name_category="category", col_name_keyword="keyword"):
            raise ValueError
    if not column_is_subset(
        df=df_2, main_col_name="category", sub_col_name="category_id", reduce=True
    ):
        raise ValueError
